...
//此处构造的事小根堆优先队列
//大根堆优先队列的定义方式应该为： priority_queue<int> q
...
void requestData::handleRequest()
{
    ...
    while (true)
    {
        ...
        {
            // 有请求出现但是读不到数据，可能是Request Aborted，或者来自网络的数据没有达到等原因。设置一个时间进行循环等待，如果超过上限就报错跳出
            perror("read_num == 0");
            if (errno == EAGAIN)
            {
...
mytimer::mytimer(requestData* _request_data, int timeout) : deleted(false), request_data(_request_data)
{
    ...
    */
    struct timeval now;
    gettimeofday(&now, NULL);           //获得微妙级的时间
    // 把当前时间统一为毫秒，并加上设置的时间上限
